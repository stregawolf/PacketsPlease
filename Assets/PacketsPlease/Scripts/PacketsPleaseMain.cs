...
public class PacketsPleaseMain : Singleton<PacketsPleaseMain> {
    ...
    protected IEnumerator HandleThrottlingTopCustomer()
    {
        ...
        ActionData action = new ActionData(m_customerListUI.GetTopCustomer(), ActionType.Throttle);
    ...
    {
        m_isHandlingCustomer = true;
        ActionData action = new ActionData(m_customerListUI.GetTopCustomer(), ActionType.Boost);
        if(RuleManager.Instance.DoesViolateRules(action))
        {
            GiveStrike();
        }
    ...
    {
        m_isHandlingCustomer = true;
        ActionData action = new ActionData(m_customerListUI.GetTopCustomer(), ActionType.Disconnect);
        if(RuleManager.Instance.DoesViolateRules(action))
        {
            GiveStrike();
        }
